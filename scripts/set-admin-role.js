#!/usr/bin/env node
/*
  Set a Supabase user's user_metadata.role = 'admin'
  Usage (PowerShell):
...
async function main() {
  ...
  const serviceKey = flagKey || process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_ROLE
  if (!url) {
  ...
  if (!serviceKey) {
    console.error('Missing SUPABASE_SERVICE_ROLE_KEY env var (or SUPABASE_SERVICE_ROLE)')
  ...
  const next = { ...current, role: 'admin' }
  console.log(`Updating user ${userId} (${user.email}) metadata ->`, next)
  ...
  console.log('Success. user_metadata:', updated.user.user_metadata)
}
